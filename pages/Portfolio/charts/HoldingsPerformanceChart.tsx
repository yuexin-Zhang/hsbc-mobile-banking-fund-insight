...
const HoldingsPerformanceChart: React.FC<HoldingsPerformanceChartProps> = ({ chartData, colors }) => {
  ...
  const [visibleLines, setVisibleLines] = useState({ fund: true, saa: true });
  ...
  const getFilteredChartData = () => {
    let startIndex = 0;
    switch (dateRange) {
      case '1M': startIndex = Math.max(0, chartData.length - 3); break;
      case '3M': startIndex = chartData.findIndex(d => d.date >= '2025-11'); if (startIndex === -1) startIndex = Math.max(0, chartData.length - 12); break;
      case '6M': startIndex = chartData.findIndex(d => d.date >= '2025-08'); if (startIndex === -1) startIndex = Math.max(0, chartData.length - 18); break;
      case '1Y': startIndex = 0; break;
      case 'YTD': startIndex = chartData.findIndex(d => d.date.startsWith('2026-01')); if (startIndex === -1) startIndex = 0; break;
    }
  ...
  const yTicks = [
    Math.ceil(yMax), Math.ceil(yMax * 0.8), Math.ceil(yMax * 0.6),
    Math.ceil(yMax * 0.4), Math.ceil(yMax * 0.2), yMin >= 0 ? 0 : Math.floor(yMin * 0.5)
  ].filter(val => !isNaN(val)).sort((a, b) => b - a);

  const getXTicksIndices = () => {
    if (filteredData.length <= 1) return [0];
    const monthsMap = new Map<string, number>();
    filteredData.forEach((d, idx) => { if (!monthsMap.has(d.date)) monthsMap.set(d.date, idx); });
    const uniqueMonths = Array.from(monthsMap.entries());
    if (dateRange === '1M' || dateRange === 'YTD') return [0, filteredData.length - 1];
    if (dateRange === '3M') {
      const indices = uniqueMonths.map(([_, idx]) => idx);
      return [...indices, filteredData.length - 1].filter((v, i, a) => a.indexOf(v) === i).sort((a, b) => a - b);
    }
    if (dateRange === '6M') {
      const indices = uniqueMonths.map(([_, idx]) => idx);
      return [...indices, filteredData.length - 1].filter((v, i, a) => a.indexOf(v) === i).sort((a, b) => a - b);
        ...
        {(['1M', '3M', '6M', '1Y', 'YTD'] as const).map((range) => (
          <button
            ...
            className={`px-3 py-1.5 text-[9px] font-bold transition-all border cursor-pointer ${
              dateRange === range ? 'bg-[#da0011] text-white border-[#da0011]' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'
            }`}
          >
            {range}
          </button>
